...
def save_final_model(model):
    if not os.path.exists('./Pong_a_model/'):
        os.mkdir('./Pong_a_model/')
    saver = tf.train.Saver()
    saver.save(model, './Pong_a_model/model.checkpoint1')

### the function to chnage image from RGB to greyscale ###
def rgb2gray(rgb):
    return np.dot(rgb[..., :3], [0.299, 0.587, 0.114])

### transfer input to size(64*64) ###
def tran_size(x, x_size, y_size):
    output = rgb2gray(x)
    output = scipy.misc.imresize(output, size=[x_size, y_size])
    return output

### stack four frames to size (64*64*4) ###
...
train_optimizer_Pong = tf.train.RMSPropOptimizer(learn_rate).minimize((q_loss))
    ...
    with tf.Session() as sess:
        ...
        for i_buffer in range(number_episode_buffer):
            ...
            for i_step in range(experience_size):
                ...
                if (i_step + 1) % 4 == 0 and i_step >= 7:
                    ...
                    sub_example.append(sum(Score[i_step + 1 - 8:i_step + 1 - 4]))
        ...
        for i_episode in range(1, episode_number_training):
            ...
            for i_training in range(training_step):
                ...
                _, loss = sess.run([train_optimizer_Pong, q_loss],
                ...
                if done is True:
                    ...
                    if (i_episode - 1) % 5 == 0:
                        print(
                            ...
                            'loss is {} ***************'.format(i_training, loss))
                        print(
