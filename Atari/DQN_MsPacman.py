...
def save_final_model(model):
    if not os.path.exists('./MsPacman_a_model/'):
        os.mkdir('./MsPacman_a_model/')
    saver = tf.train.Saver()
    saver.save(model, './MsPacman_a_model/model.checkpoint1')
...
train_optimizer_MsPacman = tf.train.RMSPropOptimizer(learn_rate).minimize((q_loss))
...
with tf.device('/cpu:0'):
    with tf.Session() as sess:
        ...
        for i_buffer in range(number_episode_buffer):
            ...
            if (i_buffer+1)%20 ==0:
                print('......the {} th episodes and  information of '
                  'observation_initial {}.....'.format(i_buffer+1,np.shape(observation_0)))
            ...
            for i_step in range(experience_size):
                ...
                #print('info score done _',score,done,_)
                ...
                if (i_step + 1) % 4 == 0 and i_step >= 7:
                    ...
                    #print('.........the sub_example.....',sub_example[2:])

                if done is True:
                    final_example = []
                    #print('......the step is {} the size of tran_size_buffer is {}......'.format(i_step + 1, np.shape(
                        #tran_size_buffer)))
            ...
            for i_training in range(training_step):
                ...
                ## update the experience ##
                    update_Action,update_Score,update_transition=update_Action[-8:],\
                                                                 update_Score[-8:],update_transition[-8:]
                ...
                ## take the randm action or greedy action ##
                    if np.random.random() <= eplison:
                        ...
                        action= int(sess.run(test_action, feed_dict={x1: stack(update_transition,8,width_size,length_size),
                                                                     keep_prob:0.8,
                ...
                _,loss = sess.run([train_optimizer_MsPacman,q_loss],feed_dict={x1: Input_1,x2:Input_2,x3:Input_3,x4: Input_4,
                ...
                if done is True:
                    ...
                    total_train_loss.append(each_episode_loss/(i_training+1))
                    experience_buffer=experience_buffer.tolist()
                    ...
                    if (i_episode - 1) % 5 == 0:
                        print('*************** the {} th step trainning'
                              ' loss is {} ***************'.format(i_training + 1, loss))
                    print('*************** the {} th episode trainning'
                    ...
                    print('************* the total trainning '
                          'time is {} 分 **************'.format(total_time/60))
                    print('========='
                          '================================================'
            ...
            if (i_episode-1)%10 == 0:
                ...
                for i_test_number in range(test_episode_number):
                    ...
                    for i_test_step in range(test_step):
                        ...
                        if test_done:
                            print('+++++++++++test {} th episode is done score is'
                                  ' {} until now +++++++++'.format(i_test_number,test_score))
                            break
                print('++++++++++++the test average score is++++++++++++++ '
                      '{}'.format(test_score/test_episode_number))
        ...
        print('agent_score',total_test_score)
        print('train_loss',total_train_loss)
        np.save('agent_score',total_test_score)
        np.save('train_loss,',total_train_loss)
