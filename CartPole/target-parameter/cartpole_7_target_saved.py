...
### the old weights ###
Weight_old_1=tf.placeholder(tf.float32,shape=[input_size,hidden_size])
Weight_old_2=tf.placeholder(tf.float32,shape=[hidden_size,output_size])
Bias_old_1=tf.placeholder(tf.float32,shape=[hidden_size])
Bias_old_2=tf.placeholder(tf.float32,shape=[output_size])
...
### calculate the target value by old weights ###
middle_next = tf.matmul(x2, Weight_old_1) + Bias_old_1
prediction_Ne = tf.nn.relu(middle_next)
prediction_next = tf.matmul(prediction_Ne, Weight_old_2) + Bias_old_2
...
with tf.device('/cpu:0'):
        ...
        with tf.Session() as sess:
            ...
            for i_eposide in range(1, 1 + eposide_size):
                ...
                for i_step in range(max_eposide_length):
                    ### hold the old weights for target calculation ###
                    if ((i_eposide - 1) % 5 == 0):
                        old_weight_1, old_weight_2, old_bias_1, old_bias_2 = sess.run([Weight_1, Weight_2, Bias_1, Bias_2])
                    ...
                    buffer_observation_now = np.append(buffer_observation_now, np.reshape(observation_0, [1, 4]),
                                                       axis=0)
                    buffer_observation_next = np.append(buffer_observation_next, np.reshape(observation_1, [1, 4]),
                                                        axis=0)
                    ...
                    _, loss_train = sess.run([train_optimizer, q_loss],
                                             feed_dict={x1: buffer_observation_now[this_batch, :],
                                                 ...
                                                 , x4: buffer_reward[this_batch],
                                                        Weight_old_1:old_weight_1,
                                                        Weight_old_2:old_weight_2,
                                                        Bias_old_1:old_bias_1,
                                                        Bias_old_2:old_bias_2,
                                                        })
                    ...
                    if (i_eposide - 1) % 20 == 0:
                        if done_0 is True:
                            ...
                            all_test_episode_length):
                                ...
                                if not os.path.exists('./part7_target/'):
                                    os.mkdir('./part7_target/')
                                saver.save(sess, "./part7_target/")
                                print('saved')
    ...
    np.save('reward_data_train_part7', outfile1)
    np.save('length_data_train_part7', outfile2)

    np.save('loss_data_train_part7', outfile3)
    np.save('length_data_test_part7', outfile4)
    np.save('reward_data_test_part7', outfile5)
