...
#### set the set to save data ####
    ...
    all_test_episode_length = np.zeros((run_size, 2000))
    all_test_reward = np.zeros((run_size, 2000))
    all_train_loss = np.zeros((run_size, 2000))
    ...
    with tf.Session() as sess:
        for i_run in range(1,run_size+1):
            ...
            for i_eposide in range(1,1+2000):
                ...
                for i_step in range(max_eposide_length):
                    ...
                    ### record average test performance ###
                        ...
                        for i_test_run in range(test_size):
                            #env.render()
                            ...
                            for i_test_length in range(max_eposide_length):
                                ...
                                if test_done is True:
                                    Small_test_eposide_length[0,i_test_run]=i_test_length+1
                                    Small_test_reward[0,i_test_run]=reward_test*(discount**(i_test_length))
                        ...
                        print('ith_run', i_run-1, 'the ith eposide', i_eposide, 'the train_length_eposide', i_step + 1,
                              'the test average length', small_mean_test_length , '..loss..',
                              train_loss)
                        all_test_episode_length[i_run-1, i_eposide-1]=small_mean_test_length
                        all_test_reward[i_run-1, i_eposide-1]=small_mean_test_reward
                        all_train_loss[i_run-1, i_eposide-1] = total_QQ_loss/(i_step+1)

                        break
                        ##### saved the model for best ...####
                if i_eposide >= 2:
                    if all_test_episode_length[i_run-1, i_eposide-1] == np.amax(all_test_episode_length):
                        print('.....',all_test_episode_length[i_run-1, i_eposide-1])
                        print(np.amax(all_test_episode_length))

                        if not os.path.exists('./part4_neural_300/'):

                            os.mkdir('./part4_neural_300/')
                        saver.save(sess, "./part4_neural_300/")
                        print('saved')
...
np.save('part4_train_reward_300', outfile1)
np.save('part4_train_eposide_length_300',outfile2)

np.save('part4_train_loss_300', outfile3)
np.save('part4_test_reward_300', outfile4)
np.save('part4_test_length_300', outfile5)
